...
/** Settings which can override default settings if exists as keys in $config['types'][$type] array
  * @var array */
    protected $typeSettings = array('disabled', 'theme', 'dirPerms', 'filePerms', 'denyZipDownload', 'maxImageWidth', 'maxImageHeight', 'thumbWidth', 'thumbHeight', 'smallSizeWidth', 'smallSizeHeight', 'mediumSizeWidth', 'mediumSizeHeight', 'largeSizeWidth', 'largeSizeHeight', 'jpegQuality', 'access', 'filenameChangeChars', 'dirnameChangeChars', 'denyExtensionRename', 'deniedExts', 'size');
...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            if (!strlen($message)) {
                ...
                if (!@move_uploaded_file($file['tmp_name'], $target) &&
                    ...
                    //if(isset($this->session['smallSizeWidth'])){
                         $this->makeSmallSize($target);
                         $this->makeMediumSize($target);
                         $this->makeLargeSize($target);
                    //}
    ...
    public function makeThumb($file, $overwrite=true) {
        $gd = new gd($file);
        ...
        if (($gd->get_width() <= $this->config['thumbWidth']) &&
            ...
            if (!$gd->resize_fit($gd->get_width(), $gd->get_height()))
                 return false;
    ...
    public function makeSmallSize($file, $overwrite=true) {
        $gd = new gd($file);
        ...
        $thumb = substr($file, strlen($this->config['uploadDir']));
        $thumb = $this->config['uploadDir'] . "/" . $this->config['smallSizeDir'] . "/" . $thumb;
        $thumb = path::normalize($thumb);
        $thumbDir = dirname($thumb);
        if (!is_dir($thumbDir) && !@mkdir($thumbDir, $this->config['dirPerms'], true))
            return false;
        ...
        // Images with smaller resolutions than thumbnails
        if (($gd->get_width() <= $this->config['smallSizeWidth']) &&
            ($gd->get_height() <= $this->config['smallSizeHeight'])
        ) {
        ...
        // Resize image
        } elseif (!$gd->resize_fit($this->config['smallSizeWidth'], $this->config['smallSizeHeight']))
            return false;
    ...
    public function makeMediumSize($file, $overwrite=true) {
        $gd = new gd($file);
        ...
        $thumb = substr($file, strlen($this->config['uploadDir']));
        $thumb = $this->config['uploadDir'] . "/" . $this->config['mediumSizeDir'] . "/" . $thumb;
        $thumb = path::normalize($thumb);
        $thumbDir = dirname($thumb);
        if (!is_dir($thumbDir) && !@mkdir($thumbDir, $this->config['dirPerms'], true))
            return false;
        ...
        // Images with smaller resolutions than thumbnails
        if (($gd->get_width() <= $this->config['mediumSizeWidth']) &&
            ($gd->get_height() <= $this->config['mediumSizeHeight'])
        ) {
            //$browsable = array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG);
            // Drop only browsable types
            //if (in_array($gd->type, $browsable))
        ...
        // Resize image
        } elseif (!$gd->resize_fit($this->config['mediumSizeWidth'], $this->config['mediumSizeHeight']))
            return false;
    ...
    public function makeLargeSize($file, $overwrite=true) {
        $gd = new gd($file);
        ...
        $thumb = substr($file, strlen($this->config['uploadDir']));
        $thumb = $this->config['uploadDir'] . "/" . $this->config['largeSizeDir'] . "/" . $thumb;
        $thumb = path::normalize($thumb);
        $thumbDir = dirname($thumb);
        if (!is_dir($thumbDir) && !@mkdir($thumbDir, $this->config['dirPerms'], true))
            return false;
        ...
        // Images with smaller resolutions than thumbnails
        if (($gd->get_width() <= $this->config['largeSizeWidth']) &&
            ($gd->get_height() <= $this->config['largeSizeHeight'])
        ) {
            //$browsable = array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG);
            // Drop only browsable types
            //if (in_array($gd->type, $browsable))
        ...
        // Resize image
        } elseif (!$gd->resize_fit($this->config['largeSizeWidth'], $this->config['largeSizeHeight']))
            return false;
