...
class browser extends uploader {
    ...
    protected $smallSizeTypeDir;
    protected $mediumSizeTypeDir;
    protected $largeSizeTypeDir;
    ...
    public function __construct() {
        ...
        $smallSizeDir = $this->config['uploadDir'] . "/" . $this->config['smallSizeDir'];
        $mediumSizeDir = $this->config['uploadDir'] . "/" . $this->config['mediumSizeDir'];
        $largeSizeDir = $this->config['uploadDir'] . "/" . $this->config['largeSizeDir'];
        ...
        $this->smallSizeDir = $smallSizeDir;
        $this->smallSizeTypeDir = "$smallSizeDir/{$this->type}";
        
        $this->mediumSizeDir = $mediumSizeDir;
        $this->mediumSizeTypeDir = "$mediumSizeDir/{$this->type}";

        $this->largeSizeDir = $largeSizeDir;
        $this->largeSizeTypeDir = "$largeSizeDir/{$this->type}";
    ...
    protected function act_renameDir() {
        ...
        $thumbDir = "$this->smallSizeTypeDir/{$this->post['dir']}";
        if (is_dir($thumbDir))
            @rename($thumbDir, dirname($thumbDir) . "/$newName");
        
        $thumbDir = "$this->mediumSizeTypeDir/{$this->post['dir']}";
        if (is_dir($thumbDir))
            @rename($thumbDir, dirname($thumbDir) . "/$newName");
        
        $thumbDir = "$this->largeSizeTypeDir/{$this->post['dir']}";
        if (is_dir($thumbDir))
            @rename($thumbDir, dirname($thumbDir) . "/$newName");
    ...
    protected function act_delete() {
        ...
        $thumb = "{$this->smallSizeTypeDir}/{$this->post['dir']}/{$this->post['file']}";
        if (file_exists($thumb)) @unlink($thumb);
        
        $thumb = "{$this->mediumSizeTypeDir}/{$this->post['dir']}/{$this->post['file']}";
        if (file_exists($thumb)) @unlink($thumb);

        $thumb = "{$this->largeSizeTypeDir}/{$this->post['dir']}/{$this->post['file']}";
        if (file_exists($thumb)) @unlink($thumb);
    ...
    protected function act_rm_cbd() {
        ...
        foreach($this->post['files'] as $file) {
            ...
            elseif (!@unlink($path))
                ...
                $thumb = "{$this->smallSizeDir}/$file";
                if (is_file($thumb)) @unlink($thumb);
                
                $thumb = "{$this->mediumSizeDir}/$file";
                if (is_file($thumb)) @unlink($thumb);
                
                $thumb = "{$this->largeSizeDir}/$file";
                if (is_file($thumb)) @unlink($thumb);
            }
        }
        if (count($error))
        ...
        foreach ($files as $file) {
            ...
            if (is_array($size) && count($size)) {
                ...
                if (!is_file($thumb_file)){
                    ...
                    //if(isset($this->session['smallSizeWidth'])){
                         $this->makeSmallSize($file, false);
                         $this->makeMediumSize($file, false);
                         $this->makeLargeSize($file, false);
                    //}
                }
                $smallThumb =
